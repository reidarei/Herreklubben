...
-- Kolonnerekken (fk, opprettet desc, id desc) dekker to behov:
--   1. Filtrering på FK + sortering på opprettet desc — det alle chat-spørringer gjør i dag.
--   2. id desc som tiebreaker — klar for tuple-cursor (opprettet, id) om vi går til
--      «last item»-paginering. Nåværende klient bruker kun opprettet-prefikset.
-- Eksisterende (fk, opprettet) asc-indekser er ikke rørt; de kan brukes av andre spørringer.
