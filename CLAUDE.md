...
Farger og tema-tokens defineres i `app/globals.css` (`:root` + `@theme inline`). `lib/tema.ts` er tynt JS-speil for manifest, epost og ICS. v1 kjører statisk `<html data-theme="dark">`. Klubb-spesifikke brand-farger settes via `NEXT_PUBLIC_KLUBB_FARGE_*`-env i `lib/klubb-config.ts` med Herreklubben-defaults.

**Regel:** Aldri hardkod hex- eller rgba-verdier i komponenter. Bruk en eksisterende `var(--token)` fra `globals.css`, eller legg til en ny token der og referer den. Tailwind-utilities (`bg-accent`, `text-text-primary`) er også tilgjengelige via `@theme inline`.

**Bevisste unntak** (dokumenterte med kommentar i koden): Avatar-hue (identitet), Ordsky/SkyBakgrunn (grafisk identitet), SladdetFelt (ren svart sladd), AlbumLightbox glass-pilknapper, RsvpBlokk indre badge-skygge, `lib/epost.ts` (e-postklienter støtter ikke CSS-vars).

Detaljer + tokenliste: [docs/tema-arkitektur.md](docs/tema-arkitektur.md).
